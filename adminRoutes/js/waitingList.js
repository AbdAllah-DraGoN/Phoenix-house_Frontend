...
window.addEventListener("scroll", () => {
  ...
  if (endOfPage && currentPage < lastPage) {
    currentPage += 1;
    getUnVerifiedUsersByAdmin(currentPage);
  }
});
// ==========================================
function getUnVerifiedUsersByAdmin(page = 1, reload = false) {
  const token = localStorage.getItem("Is_My_Token");
  toggleLoader();
  axios
    .get(`${baseUrl}/admin/user/unverified?limit=10&page=${page}`, {
      headers: {
        "x-api-key": apiKey,
    ...
    .then((response) => {
      ...
      users.forEach((user) => {
        userCard(user.id, user.name, user.phone, user.createdAt);
      });
...
function userCard(id, name, phone, createAt) {
  let createTime =
...
function confirmDeleteUser() {
  ...
  axios
    ...
    .then((response) => {
      ...
      getUnVerifiedUsersByAdmin(1, true);
    })
    .catch((error) => {
      toggleLoader(false);
      console.log(error);
...
function verifyUser(id) {
  ...
  toggleLoader();
  axios
    .post(`${baseUrl}/admin/user/verify/${id}`, params, {
      headers: {
        "x-api-key": apiKey,
        authorization: `Bearer ${token}`,
      },
    ...
    .then((response) => {
      toggleLoader(false);
      // console.log(response);
      showAlert("تم قبول المستخدم المستخدم بنجاح");
      getUnVerifiedUsersByAdmin(1, true);
    })
    .catch((error) => {
      toggleLoader(false);
